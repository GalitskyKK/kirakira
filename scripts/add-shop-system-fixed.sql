-- =============================================
-- ИСПРАВЛЕННАЯ МИГРАЦИЯ: Добавление системы магазина с RLS
-- =============================================
...
-- 4. Создаем RLS политики для shop_purchases
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'shop_purchases' 
    AND policyname = 'Users can insert own purchases'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    ...
    WHERE schemaname = 'public' 
    AND tablename = 'shop_purchases' 
    AND policyname = 'Users can update own purchases'
  ) THEN
...
-- 5. Проверяем и включаем RLS для user_currency (если не включен)
ALTER TABLE public.user_currency ENABLE ROW LEVEL SECURITY;

-- Политика SELECT для user_currency (если не существует)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'user_currency' 
...
-- Политика UPDATE для user_currency (если не существует)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'user_currency' 
    AND policyname = 'Users can update own currency'
  ) THEN
...
-- 6. Проверяем и включаем RLS для currency_transactions
ALTER TABLE public.currency_transactions ENABLE ROW LEVEL SECURITY;

-- Политика SELECT для currency_transactions (если не существует)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 
    WHERE schemaname = 'public' 
    AND tablename = 'currency_transactions' 
...
-- Политика INSERT для currency_transactions (если не существует)
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies 

...
export interface VibeConfig {
  ...
  circularMask?: boolean // Enable soft circular mask for companions
}
...
export class VibeController {
  ...
  updateConfig(config: VibeConfig) {
    ...
    const newBlobCount =
      config.blobCount ?? config.colors?.length ?? this.blobCount
  ...
  private render() {
    ...
    if (vScreenSize)
      this.gl.uniform2f(vScreenSize, this.canvas.width, this.canvas.height)

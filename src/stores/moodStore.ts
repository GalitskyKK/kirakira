...
export const useMoodStore = create<MoodStore>()(
  subscribeWithSelector(
    persist(
      (set, get) => ({
        ...
        loadMoodHistory: () => {
          ...
          try {
            ...
            set({
              ...
              lastCheckin: lastEntry?.date ?? get().lastCheckin, // 🔥 ИСПРАВЛЕНИЕ: Используем сохраненный lastCheckin
              isLoading: false,
            })
        ...
        syncMoodHistory: async (forceSync = false) => {
          try {
            ...
            console.log(
              `📡 Fetching user profile for ${currentUser.telegramId}...`
            )
          ...
          try {
            ...
            if (localSuccess) {
              ...
              try {
                ...
                if (!response.ok) {
                  console.warn(
                    '⚠️ Failed to sync mood to server:',
                    response.status
                  )
                } else {
        ...
        canCheckinToday: () => {
          ...
          if (gardenStore.currentGarden) {
            ...
            if (todayElement) {
              console.log(
                '🌱 Found garden element for today - mood already marked'
              )
      ...
      {
        name: 'kirakira-mood-storage', // Имя для localStorage
        partialize: state => ({
          lastCheckin: state.lastCheckin, // Сохраняем ТОЛЬКО lastCheckin
        }),
      }
    )
  )
